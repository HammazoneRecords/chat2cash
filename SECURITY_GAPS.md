...
| FIX-012 | `[?] Needs proof` | MindWave buyer payout model | Updated payout version to `c2c-payout-v6-mindwave-buyer` with JMD 25/50/75/100 tier rates and JMD 200 max displayed rate per accepted pair. | Local source: `lib/contextGrading.ts`, `LandingHero.tsx`, `HelpFaq.tsx`; unit test. Browser/live proof still required. |
...
| FIX-021 | `[?] Needs proof` | Environment template | `.env.example` now matches Chat2Cash local/production setup, Better Auth URL requirements, JMD launch payout country, DATA_DIR, AI provider, and test-account seed variables without secrets. | Local source: `.env.example`; unit invariant test. Operator/live env proof still required. |
| FIX-022 | `[?] Needs proof` | Scoring fixture coverage | Added deterministic grading signals and fixtures for follow-up context, topic shifts, contradiction, Patois/code-switching, spelling variation, and creative/cultural insight. | Local source: `lib/contextGrading.ts`, `tests/contextGrading.test.ts`. Browser/API ZIP proof still required. |
...
| SEC-011 | `[x] Done` | Medium | No | Duplicate / Strike UX | ZIP/TXT and JSON drafts expose duplicate status before final submit; final duplicate submission still follows the strike policy. | `server.ts` returns `duplicatePreview`; `FileProcessor.tsx` displays the warning before submit. | Keep the warning aligned with the strike policy. | Uploading an already-submitted JSON/ZIP shows the strike or partial-duplicate result before final submit. |
| SEC-012 | `[?] Needs proof` | Medium | No | Legacy data | Fixed locally: maintenance script is dry-run by default, requires `--apply`, backs up the DB, and backfills zero-price pending datasets with current buyer-pricing metadata. | `scripts/backfill-zero-pricing.cjs`, invariant test. | Run dry-run/apply against a copied production DB after backup and record proof. | Dry run lists affected dataset IDs; apply mode updates pricing/labels/metadata without changing raw user ownership. |
| SEC-013 | `[?] Needs proof` | Medium | No | Scoring integrity | Fixed locally: deterministic grading now recognizes clear Patois/code-switching, spelling variation, and creative/cultural insight; fixtures cover follow-up context, topic shift, contradiction, Patois, spelling variation, and creative/cultural insight. | Local source: `lib/contextGrading.ts`, `tests/contextGrading.test.ts`. | Browser/API ZIP review checks still required against real uploads. | Release gate includes fixtures for topic shift, follow-up context, contradiction, Patois, spelling variation, and creative/cultural insight. |
...
| SEC-017 | `[x] Done` | Medium | No | Operational scripts | `scripts/backfill-zero-pricing.cjs` is now dry-run by default, requires explicit `--apply`, creates a timestamped backup, and wraps writes in a transaction. | Local source: `scripts/backfill-zero-pricing.cjs`; invariant test. | Keep `--apply` guarded and back up before writes. | Running without `--apply` changes zero rows and prints intended changes only. |
| SEC-018 | `[?] Needs proof` | Low | No | Config docs | Fixed locally: `.env.example` now uses Chat2Cash local/production URLs, `DATA_DIR`, `WIPAY_COUNTRY_CODE=JM`, valid Better Auth secret generation guidance, and test account seed variables. | Local source: `.env.example`; unit invariant test. | Confirm live server env mirrors the documented required values after deploy. | New operator can seed accounts and run production build without Better Auth base URL warning. |
| SEC-019 | `[?] Needs proof` | Medium | No | Legacy profile data | Fixed locally: maintenance script detects legacy base64 `profiles.idPhoto` rows and can replace them with `verified-hash:v1` markers after backup. | `scripts/backfill-zero-pricing.cjs`, invariant test. | Run dry-run/apply against copied/live DB after backup and record proof. | Dry run lists affected profile count; apply removes all `data:image/*;base64` values from `profiles.idPhoto`. |
...
| 2026-07-13 | FIX-021/SEC-018 | Replaced stale env template values with Chat2Cash production/local URLs, JMD WiPay country, DATA_DIR, AI provider, Better Auth secret guidance, and test seed variables. |
| 2026-07-13 | FIX-022/SEC-013 | Added scoring fixtures and deterministic language/cultural signals for follow-up context, topic shifts, contradiction, Patois/code-switching, spelling variation, and creative/cultural insight. |
