...
| SEC-019 | `[ ] Open` | Medium | No | Legacy profile data | New profile updates no longer store base64 ID images, but existing production rows may already contain base64 `profiles.idPhoto` values from previous builds. | Local source fix is forward-looking; no migration has been run against live DB in this turn. | Add dry-run/apply migration that replaces legacy base64 ID images with hash markers after backup. | Dry run lists affected profile count; apply removes all `data:image/*;base64` values from `profiles.idPhoto`. |
