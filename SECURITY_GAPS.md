...
| SEC-003 | `[ ] Open` | High | Yes | Auth / Admin | Legacy picture-password admin auth still exists beside Better Auth staff roles. It can auto-create/sign in an admin account if the passphrase is configured. | Local source: `/api/admin/picture-verify` and `/api/admin/auth` in `server.ts`. | Remove the legacy picture-password path or restrict it behind owner-only setup mode that is disabled in production. | Unauthenticated calls to `/api/admin/picture-verify` and `/api/admin/auth` return 404 or disabled response in production. |
...
| SEC-006 | `[ ] Open` | Medium | No | Public copy | Ledger and landing copy still mention XCD, identity indexes, harvested dial datasets, and on-chain style wording while code now uses JMD and anonymous public receipts. | Local source: `ReconciliationLedger.tsx` displays `XCD`; landing copy references `UserId` index. | Update copy to match actual privacy and payout behavior. | Public UI says JMD, anonymous receipt, 7-14 day review, and no identity keys/on-chain claims. |
| SEC-007 | `[ ] Open` | High | Yes | Privacy / Admin exports | Admin export endpoints return full dataset objects or JSONL including `userId`, `dialogues`, and metadata. This may be acceptable for admin-only export, but needs explicit safe-export policy and proof that no raw timestamps/original lines/PII survive. | Local source: `/api/admin/datasets/:id/export` and `/api/admin/export-all` in `server.ts`. | Add export sanitization contract and tests; export only anonymized dialogue and necessary metadata by default. | Tests prove exports contain no `originalLine`, absolute timestamps, raw names, emails, phone numbers, or contributor contact fields unless an owner-only operational export explicitly requires them. |
