...
function evaluateDialogueLocally(d: { prompt: string; response: string }) {
  ...
  // 4. Regional slang / Patois check. Dialect is valuable when there is enough context.
  ...
  if ((hasSlangPrompt && promptWords.length < 5) || (hasSlangResponse && responseWords.length < 5)) {
    return {
      isUseful: false,
      score: 20,
      category: "Needs More Context",
      explanation: "Local Rule: Patois or slang detected, but the turn is too short to preserve meaning by itself."
    };
  }
...
2. Patois, dialect, slang, code-switching, and spelling variation: score on meaning, context, and usefulness. Do not reject a turn just because it is dialect-heavy. If the meaning is too short or unclear without surrounding context, use category = "Needs More Context".
