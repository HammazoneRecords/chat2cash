...
| AUD-014 | Medium | Stats semantics | Fixed locally: Overview now shows dataset payout totals as `Payouts Under Review` and preserves transaction-based `totalPaidJMD` for actual paid totals. | `db.ts getStats()` returns both `totalPayoutsUnderReviewJMD` and `totalPaidJMD`; `LandingHero.tsx` uses the review label. | Browser/live proof still required. | Stats labels match query semantics. |
