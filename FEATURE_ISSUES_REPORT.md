...
| UX-004 | `[x] Done` | P0 | Pricing clarity | Pricing copy conflicted between old low-rate language and the current MindWave buyer model. | FAQ, hero, and backend now state JMD $10-$75 per accepted chat pair with tiered buyer rates and cap. | Keep pricing table consistent across all copy. | Source check confirms the same rate language in Overview and FAQ. |
...
| UX-014 | `[~] In progress` | P1 | ID redaction | Backend now stores only a one-way ID verification marker for new multiplier opt-ins and strips `idPhoto` from profile responses, but frontend copy still needs clearer “skip multiplier” explanation. | Source: `server.ts`, `RegistrationForm.tsx`. | Add concise opt-in explanation and “skip multiplier” path. | User can clearly choose standard payout without ID. |
...
| 2026-07-13 | UX-014 partial | Backend profile update now validates ID image type/size and stores only a one-way verification marker; frontend explanation still needs work. |
