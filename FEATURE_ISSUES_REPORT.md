...
| ID | Status | Priority | Feature | User-facing issue | Evidence | Suggested fix | Acceptance check |
|---|---|---:|---|---|---|---|---|
| UX-013 | `[?] Needs proof` | P1 | Signup friction | Fixed locally: registration, ZIP/JSON review, and final dataset submission work without WiPay; payout request and admin payout queue require a complete WiPay payout profile. | Source: `RegistrationForm.tsx`, `FileProcessor.tsx`, `server.ts`, invariant test. | Browser-test signup, ZIP preview, submit without WiPay, then payout setup. | User can submit without payout details and receives a clear payout-setup requirement only when payment becomes eligible. |
...
| UX-016 | `[?] Needs proof` | P2 | Password errors | Fixed locally: login and registration now normalize common auth/profile failures into safe next-action messages for wrong password, existing email, profile load, and profile field correction. | Source: `RegistrationForm.tsx`, `LoginForm.tsx`, invariant test. | Browser-test failed login, existing-email signup, and profile validation errors after deploy. | Common auth failures have clear next action. |
...
| UX-024 | `[?] Needs proof` | P1 | Evidence review depth | Fixed locally: admin review now shows sanitized message snippets for each segment plus per-dimension score, confidence, source, and evidence message IDs. | Source: `AdminDashboard.tsx`, invariant test. | Browser-test admin review panel after deploy. | Moderator can justify a decision without exporting JSON. |
...
| 2026-07-13 | UX-024 | Added sanitized segment snippets and score-dimension evidence message IDs to the admin review panel. |
